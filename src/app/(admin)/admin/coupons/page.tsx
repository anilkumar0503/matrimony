        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={7} className="text-center py-12 text-muted">No coupons found</td></tr>
            ) : coupons.map((c) => (
              <tr key={c.id} className="border-b border-border hover:bg-white/[0.02]">
                ...
                <td className="py-3 px-4 text-muted text-xs">{formatDate(c.validUntil)}</td>
          ...
          <div className="glass-dark p-6 rounded-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs text-muted mb-1.5">Coupon Code *</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs text-muted mb-1.5">Discount Value *</label>
                ...
                <div>
                  <label className="block text-xs text-muted mb-1.5">Usage Limit</label>
              ...
              <div>
                <label className="block text-xs text-muted mb-1.5">Applicable Plans (leave empty for all)</label>
                <div className="flex flex-wrap gap-2">
                  {plans.map(p => (
                    <label key={p.id} className="flex items-center gap-1.5 text-xs text-muted cursor-pointer">
                      <input
                        type="checkbox"
