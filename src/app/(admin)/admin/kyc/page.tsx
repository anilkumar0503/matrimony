...
export default function AdminKycPage() {
  ...
  return (
    <div className="space-y-6">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={7} className="px-4 py-12 text-center text-muted text-sm">No {statusFilter.toLowerCase()} submissions</td></tr>
              ) : submissions.map((sub) => (
                <tr key={sub.id} className={`border-b border-border hover:bg-surface-hover ${sub.isSlaBreach ? "bg-amber-500/5" : ""}`}>
                  <td className="px-4 py-3">
                    ...
                    <div className="text-muted text-xs">{sub.user.gender} · {sub.user.dateOfBirth ? calculateAge(sub.user.dateOfBirth) + " yrs" : "—"}</div>
                  </td>
                  <td className="px-4 py-3">
                    <Badge variant="glass">{sub.mode || "Mode A"}</Badge>
                  </td>
                  <td className="px-4 py-3 text-muted text-xs">{formatDateTime(sub.createdAt)}</td>
                  <td className="px-4 py-3">
                    <span className={`text-sm font-medium ${sub.attempts >= 3 ? "text-red-600 dark:text-red-400" : "text-muted"}`}>
                      {sub.attempts}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-1">
                      ...
                      {sub.status === "PENDING" && (
                        <>
                          <Button variant="glass" size="sm" className="h-7 text-xs text-emerald-600 dark:text-emerald-400"
                            onClick={() => { setSelected(sub); setAction("APPROVE"); }}>
                            <CheckCircle size={12} /> Approve
                          </Button>
                          <Button variant="glass" size="sm" className="h-7 text-xs text-red-600 dark:text-red-400"
      ...
      {/* Review modal */}
      {selected && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 overflow-y-auto" onClick={() => { setSelected(null); setAction(null); }}>
          <div className="bg-background border border-border rounded-2xl p-6 max-w-2xl w-full my-4 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-start justify-between mb-6">
              <div>
                <h3 className="font-semibold text-foreground text-lg">{selected.user.profile?.fullName || "Unknown"}</h3>
                <p className="text-muted text-sm">{selected.user.email} · {selected.user.gender} · Attempt #{selected.attempts}</p>
              </div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div>
                <div className="text-muted text-xs mb-2">Verification Code</div>
                <div className="glass p-3 font-mono text-[#E8C76A] text-lg tracking-widest">{selected.verificationCode}</div>
              </div>
              <div>
                <div className="text-muted text-xs mb-2">Submitted At</div>
                <div className="text-muted text-sm">{formatDateTime(selected.createdAt)}</div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              {selected.selfieUrl && (
                <div>
                  <div className="text-muted text-xs mb-2">Selfie with Code</div>
              ...
              {selected.idDocumentUrl && (
                <div>
                  <div className="text-muted text-xs mb-2">ID Document</div>
                  <div className="aspect-square bg-surface rounded-xl overflow-hidden flex items-center justify-center border border-border">
              ...
              {!selected.selfieUrl && !selected.idDocumentUrl && (
                <div className="col-span-2 text-center text-muted text-sm py-8 border border-border rounded-xl">
            ...
            {selected.user.images.length > 0 && (
              <div className="mb-6">
                <div className="text-muted text-xs mb-2">Profile Photos (for comparison)</div>
              ...
              <>
                {action === "REJECT" && (
                  <div className="space-y-3 mb-4">
                    ...
                    <div>
                      <label className="block text-xs text-muted mb-1.5">Additional Notes</label>
