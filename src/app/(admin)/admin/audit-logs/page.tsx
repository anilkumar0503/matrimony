            ...
            <thead>
              <tr className="border-b border-border">
                {["Timestamp", "Actor", "Action", "Target", "IP Address", "Details"].map((h) => (
                  <th key={h} className="text-left px-4 py-3 text-muted text-xs font-semibold uppercase tracking-wider align-middle">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={6} className="px-4 py-12 text-center text-muted align-middle">No audit logs found</td></tr>
              ) : logs.map((log) => {
                const actionColor = Object.entries(ACTION_COLORS).find(([k]) => log.action.toUpperCase().includes(k))?.[1] || "text-muted";
                return (
                  <tr key={log.id} className="border-b border-border hover:bg-white/[0.015]">
                    <td className="px-4 py-3 text-muted whitespace-nowrap align-middle">{formatDateTime(log.createdAt)}</td>
                    <td className="px-4 py-3 align-middle">
                      {log.admin ? (
                        <div>
                          <div className="text-muted font-medium">{log.admin.name}</div>
                          <div className="text-muted text-[10px]">{log.admin.email}</div>
                        </div>
                    ...
                    <td className="px-4 py-3 text-muted align-middle">
                      ...
                      {log.targetId && <div className="text-muted mt-0.5 font-mono">{log.targetId.slice(0, 8)}…</div>}
                    </td>
                    <td className="px-4 py-3 text-muted font-mono align-middle">{log.ipAddress || "—"}</td>
                    <td className="px-4 py-3 text-muted max-w-xs truncate align-middle">
