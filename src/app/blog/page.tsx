              ...
              <Link key={post.slug} href={`/blog/${post.slug}`} className="group">
                <article className="glass overflow-hidden h-full flex flex-col hover:border-[rgba(201,151,44,0.25)] transition-colors p-0">
                  ...
                  <div className="p-5 flex flex-col flex-1">
                    ...
                    <h2 className="font-display text-lg font-bold text-white mb-2 leading-tight group-hover:text-[#E8C76A] transition-colors line-clamp-2">
                      {post.title}
                    </h2>
                    ...
                    <div className="mt-4 flex items-center justify-between">
                      <span className="text-white/30 text-xs">{post.author?.name}</span>
                    ...
                    {post.tags?.length > 0 && (
                      <div className="flex flex-wrap gap-1.5 mt-3">
                        {post.tags.slice(0, 3).map((tag: string) => (
                          <span key={tag} className="text-[10px] text-white/30 bg-white/[0.04] border border-white/[0.06] px-2 py-0.5 rounded-full flex items-center gap-0.5">
                            <Tag size={8} /> {tag}
                          </span>
                        ))}
                      </div>
