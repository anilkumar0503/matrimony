    ...
    <>
      ...
      {communities.length === 0 ? (
        <div className="glass p-8 text-center">
          ...
          <h3 className="font-display text-xl text-foreground mb-2">No Communities Yet</h3>
          <p className="text-muted text-sm">Communities will be listed here once they are created.</p>
        </div>
      ) : (
        <div className="grid md:grid-cols-2 gap-6">
          {communities.map((community) => (
            <div key={community.id} className="glass p-6 hover:border-border transition-colors">
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-display text-lg font-bold text-foreground">{community.name}</h3>
                    <Badge variant="glass" className="text-xs">{community.category}</Badge>
                  </div>
                </div>
              </div>

              <p className="text-muted text-sm mb-4 line-clamp-2">{community.description}</p>
